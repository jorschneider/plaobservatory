...
The release described here has a data cutoff of 2 September 2026 and the build ID `PLA26-V8-14E18B4FCE64` (app/data/observatory.json, `metadata`). Rerunning the build on the same inputs gives the same ID.
...
- The disappearance clock. Of the 63 ledger records, 8 are complete, 45 are partial and 10 are not yet collected (app/data/observatory.json, `metadata.ledgerClock.counts`). Complete means all three dates are present and official. Partial means at least one of the three is present and official. Most partial records have only the formal action, because the NPC join supplied it. Ten records have a last public appearance (`metadata.ledgerClock.withLastAppearance`), each with the exact Chinese title, the event, the URL, and a note saying the page was fetched and read (research/adverse-timeline.json, `records[].lastPublicAppearance`). Over the 8 complete records, the median gap from last appearance to first concrete signal is 86 days, with a range of 30 to 382. Over the 12 records with both an official first signal and an official formal action, the median gap from first signal to formal action is 233 days, with a range of 134 to 861 (`metadata.ledgerClock.silence` and `metadata.ledgerClock.process`). In the research file, 53 records have a formal action, of which 14 are only a secondary classification; 21 have a first concrete signal, of which 7 are only a secondary classification (research/adverse-timeline.json, `records[].formalAction.kind` and `records[].firstConcreteSignal.kind`). Nine records carry the search-lane note "Not researched in this pass", and 10 carry research notes from the pass.
