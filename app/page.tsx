...
type Trackers = {
  ...
  eventAttendance?: { events: { family: string; date: string; titleZh: string; url: string; rosterComplete: string; namedMilitaryAttendees: { nameZh: string; nameEn: string; titleZh: string }[] }[]; families: string[]; misses: { family: string; nameZh: string; nameEn: string; expectedFrom: string; missedAt: string; missedEventUrl: string; match: { kind: string; id: string } | null; laterResolution: { status: string; formalActionDate: string | null; firstSignalDate: string | null } | null }[]; summary: { events: number; families: number; completeRosters: number; misses: number; missesLaterConfirmedAdverse: number; missesWithNoAdverseRecord: number }; scoredFamilies?: Record<string, number>; gaps: string[] };
  discoveredNames?: { summary: { names: number; fromCentralCommittee: number; seenTwiceOrMore: number }; leads: { nameZh: string; nameEn: string; sightings: { where: string; detail: string; url: string }[] }[] };
...
const familyLabel: Record<string, string> = { cmc_tree_planting: "CMC tree-planting", august_first_reception: "1 August reception", npc_military_delegation: "NPC military delegation plenary", npc_military_delegation_other: "NPC military delegation (other)", cc_plenum: "Central Committee plenum", promotion_ceremony: "Promotion ceremony", other_cmc_event: "Other CMC-level event" };
...
const judgmentVars = (() => {
  ...
  return {
    ...
    turnoverSentence: st ? `Of the ${st.summary.turnovers} senior seats that turned over since 2023, ${st.summary.closed} were filled by a formally named successor (${st.turnovers.filter((x) => x.closed && x.daysOpen === 0).length} of them on the day the predecessor left) and ${st.summary.open} are still open${st.summary.medianDaysOpenStillVacant !== null ? `, for a median of ${st.summary.medianDaysOpenStillVacant} days so far` : ""}.` : "The seat-turnover tracker is still being collected.",
    ...
    attendanceEvents: data.trackers.eventAttendance?.summary.events ?? null, completeRosters: data.trackers.eventAttendance?.summary.completeRosters ?? null, scoredMisses: data.trackers.eventAttendance?.summary.misses ?? null, missesAdverse: data.trackers.eventAttendance?.summary.missesLaterConfirmedAdverse ?? null, missesUnexplained: data.trackers.eventAttendance?.summary.missesWithNoAdverseRecord ?? null,
          ...
          <TabsContent value="trackers" className="tab-panel">
            ...
            <section className="tracker-section">
              ...
              {t.eventAttendance ? <>
                ...
                {t.eventAttendance.misses.length ? <><h4 className="tracker-subhead">Scored misses (candidate signals, not findings)</h4><p className="signal-method-note">A miss is scored only in a family with a stable roster: one prior appearance for the CMC tree-planting and the 1 August reception, two prior appearances for the NPC delegation plenary and the promotion ceremonies. {t.eventAttendance.summary.missesLaterConfirmedAdverse} of {t.eventAttendance.summary.misses} scored misses belong to officers who later entered the adverse ledger; the rest are unexplained and stay unexplained.</p><div className="architecture-table"><Table><TableHeader><TableRow><TableHead>Officer</TableHead><TableHead>Event</TableHead><TableHead>Present</TableHead><TableHead>Missed</TableHead><TableHead>What happened next</TableHead></TableRow></TableHeader><TableBody>{t.eventAttendance.misses.map((x, i) => <TableRow key={`${x.nameZh}-${x.missedAt}-${i}`}><TableCell><strong>{x.nameEn}</strong> <span lang="zh-Hans">{x.nameZh}</span></TableCell><TableCell>{familyLabel[x.family] || x.family}</TableCell><TableCell className="mono">{x.expectedFrom}</TableCell><TableCell className="mono">{x.missedAt}</TableCell><TableCell>{x.laterResolution ? <span className="signal-chip signal-stale">{x.laterResolution.status}{x.laterResolution.formalActionDate ? ` · formal action ${x.laterResolution.formalActionDate}` : ""}</span> : <span className="signal-chip signal-missing">No adverse record</span>}</TableCell></TableRow>)}</TableBody></Table></div></> : null}
            ...
            <section className="tracker-section">
              <div className="section-heading-row"><div><p className="section-kicker">Tracker 7</p><h3>Names the trackers surfaced that the dataset does not have</h3><p>Officers named in the Central Committee list, a promotion ceremony, an NPC termination, a seat turnover or a complete event roster who are in no dossier and no ledger record. These are the leads for the seats marked no record.</p></div><Badge variant="outline">{t.discoveredNames ? `${t.discoveredNames.summary.names} names · ${t.discoveredNames.summary.fromCentralCommittee} on the Central Committee` : "Not yet computed"}</Badge></div>
              {t.discoveredNames ? <div className="architecture-table"><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Where seen</TableHead></TableRow></TableHeader><TableBody>{t.discoveredNames.leads.map((l) => <TableRow key={l.nameZh}><TableCell><strong>{l.nameEn || "—"}</strong> <span lang="zh-Hans">{l.nameZh}</span></TableCell><TableCell>{l.sightings.map((x, i) => <span key={i} className="clock-event"><small>{x.where}: {x.detail}</small>{x.url ? <a href={x.url} target="_blank" rel="noreferrer">{sourceHost(x.url)} <ArrowUpRight /></a> : null}</span>)}</TableCell></TableRow>)}</TableBody></Table></div> : null}
            </section>
          </TabsContent>
