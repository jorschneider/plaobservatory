...
type Scorecard = { id: string; supplier: string; englishName: string | null; capabilityFamily: string; nodeId: string; lane: string; status: string; rankEligible: boolean; tier: string; criticality: number | string; frontier: number | string; crossDomain: number | string; evidence: string; maturity: string; importance: number; lowerBound: number; evidenceCount: number; caveat: string | null; initialRank: number | null; fragility: string | null; fragilityBasis: string | null; sourceUrls: string[] };
type Node = { id: string; tier: string; label: string; plain: string; capabilityFamilies: string[]; coverage: string; suppliers: { id: string; supplier: string; englishName: string | null; tier: string; importance: number; lane: string }[]; evidenceQualifiedCount: number; provisionalCount: number };
type Data = {
...
const coverageLabel: Record<string, string> = { evidence_qualified: "Evidence-qualified supplier", provisional_only: "Provisional records only", no_record: "No evidence yet" };
const coveragePlain: Record<string, string> = { evidence_qualified: "At least one scorecard on this node has E3 or E4 evidence and is rank-eligible.", provisional_only: "Firms are named against this node, but only from self-description, pre-award notices or comparative tests.", no_record: "No firm is evidenced against this node. The node stays on the chart so the gap is visible." };
...
export default function RoboticsLane() {
  ...
  const cards = useMemo(() => { const q = query.trim().toLowerCase(); return data.scorecards.filter((c) => (lane === "all" || c.lane === lane) && (tierFilter === "all" || c.tier === tierFilter) && (!q || `${c.supplier} ${c.englishName ?? ""} ${c.capabilityFamily} ${c.nodeId}`.toLowerCase().includes(q))).sort((a, b) => (b.importance ?? 0) - (a.importance ?? 0)); }, [lane, tierFilter, query]);
    ...
    <main className="site-shell">
      ...
      <div id="top" className="workspace">
        <section className="briefing-header">
          ...
          <aside className="editorial-note"><Scale aria-hidden="true" /><div><strong>What this lane does not do</strong><p>It publishes no revenue, volume or wartime-indispensability claims. Systemic importance is a bounded triage score; only evidence-qualified scorecards are ranked; everything else is labeled provisional.</p></div></aside>
        </section>
        <Tabs value={tab} onValueChange={setTab} className="main-tabs">
          <div className="tabs-bar"><TabsList variant="line" className="tabs-list"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="nodes">Nodes</TabsTrigger><TabsTrigger value="scorecards">Scorecards</TabsTrigger><TabsTrigger value="evidence">Evidence</TabsTrigger><TabsTrigger value="trackers">Trackers</TabsTrigger><TabsTrigger value="method">Method</TabsTrigger></TabsList></div>
          ...
          <TabsContent value="overview" className="tab-panel">
            ...
            <section className="judgment-stack judgment-stack-priority">
              <div className="section-heading-row"><div><p className="section-kicker">What the chart shows</p><h3>Findings computed from the scorecards and the ledger</h3></div><Badge variant="outline">Counts, not opinions</Badge></div>
          ...
          <TabsContent value="nodes" className="tab-panel">
            <section className="panel-heading"><div><p className="section-kicker">Capability chart</p><h2>Every node a robotic system depends on, and who is evidenced against it</h2><p>{data.chart.plain}</p></div><Badge variant="outline">{m.nodeCount} nodes · chart {m.chartVersion}</Badge></section>
          ...
          <TabsContent value="scorecards" className="tab-panel">
            ...
            <div className="architecture-table scorecard-table"><Table><TableHeader><TableRow><TableHead>Rank</TableHead><TableHead>Supplier</TableHead><TableHead>Capability · node</TableHead><TableHead>Lane</TableHead><TableHead>Tier</TableHead><TableHead>C / I / X / E</TableHead><TableHead>Maturity</TableHead><TableHead className="text-right">Importance</TableHead><TableHead>Caveat</TableHead><TableHead>Sources</TableHead></TableRow></TableHeader><TableBody>{cards.map((c) => <TableRow key={c.id}><TableCell className="mono">{c.tier === "evidence_qualified" && c.initialRank ? `#${c.initialRank}` : "—"}</TableCell><TableCell><strong>{c.englishName || c.supplier}</strong><br /><small lang="zh-Hans">{c.supplier}</small></TableCell><TableCell>{fam(c.capabilityFamily)}<br /><button className="position-link" onClick={() => openNode(c.nodeId)}>{data.chart.nodes.find((n) => n.id === c.nodeId)?.label}</button></TableCell><TableCell>{laneLabel[c.lane] ?? c.lane}</TableCell><TableCell><span className={`status-pill ${tierTone[c.tier]}`}>{tierLabel[c.tier]}</span></TableCell><TableCell className="mono">{c.criticality} / {c.frontier} / {c.crossDomain} / {c.evidence}</TableCell><TableCell className="mono">{c.maturity}</TableCell><TableCell className="text-right mono">{c.importance}<br /><small>floor {c.lowerBound}</small></TableCell><TableCell><small>{c.caveat}{c.fragility ? ` Fragility: ${c.fragility}.` : ""}</small></TableCell><TableCell>{c.sourceUrls.slice(0, 3).map((u) => <a key={u} href={u} target="_blank" rel="noreferrer" className="position-link">{host(u)} <ArrowUpRight /></a>)}</TableCell></TableRow>)}</TableBody></Table></div>
          </TabsContent>

          <TabsContent value="evidence" className="tab-panel">
            <section className="panel-heading"><div><p className="section-kicker">Evidence ledger</p><h2>Every scorecard links here</h2><p>Each row is one dated claim from one source, with what it establishes and what it does not.</p></div><Badge variant="outline">{evidence.length} of {m.evidenceRowCount}</Badge></section>
